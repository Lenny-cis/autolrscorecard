...
class VarBinning:
    ...
    def cal_sort_values(self):
        ...
        return dd.to_dict(orient='index')
    ...
    def grid_search_best(self, verbose=True, bins_cnt=None,
        ...
        with make_tqdm_iterator(**tqdm_options) as progress_bar:
            for prod in prod_:
                ...
                progress_bar.update()
        self.best_bins = best_dd
        return self

    def manual_select(self, best_key):
        self.selected_best = deepcopy(self.best_bins[best_key])
        return self

    def fast_search_best(self, slc_mthd='IV', tolerance=0.1, **kwargs):
        if slc_mthd != 'IV':
        ...
        if len(dic) == 0:
            return self
        dd = pd.DataFrame.from_dict(dic, orient='index')
        tem_dd = dd.loc[dd.loc[:, 'tolerance'] > tolerance]
        tem_dic = tem_dd.sort_values(by=slc_mthd, ascending=False)\
            .iloc[0, :].to_dict()
        self.selected_best = tem_dic
        return self
    ...
    def plot_best(self):
        ...
        for best_key, best_val in self.best_bins.items():
...
class VarBinning2:
    ...
    def __init__(self, subjective_cut=None, variable_type=None, **kwargs):
        self.variable_type = type(variable_type)
        self.describe = kwargs.get('describe', '未知')
        self.subjective_cut = subjective_cut
        self.data = {'best_bins': {}, 'bins_set': {}}
    ...
    def fit(self, x, y):
        ...
        self.indep = x.name
        if issubclass(self.variable_type, (vtype.Summ, vtype.Count)):
            self.quantile = np.percentile(x.dropna(), [0, 25, 50, 75, 100])\
                .tolist()
        else:
            self.quantile = []
        ...
        self.best_bins = deepcopy(self.bins_set)
        return self

    def transform(self, x):
        """单变量应用."""
        trns = pd.DataFrame()
        for i in self.best_bins:
            shape = self.best_bins[i]['shape']
            bin_cnt = self.best_bins[i]['bin_cnt']
            cut = self.best_bins[i]['cut']
            woe = self.best_bins[i]['WOE']
            name = '_'.join([str(x.name), str(shape), str(bin_cnt)])
            x_trns = apply_woe(x, cut, woe, self.variable_type)
            x_trns.name = name
            trns = pd.concat([trns, x_trns], axis=1)
